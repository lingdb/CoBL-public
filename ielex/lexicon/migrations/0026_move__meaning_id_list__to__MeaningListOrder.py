...
from ielex import lexicon

class Migration(DataMigration):

    def forwards(self, orm):
        # Note: Remember to use orm['appname.ModelName'] rather than "from appname.models..."
        """For each MeaningList object, create a series of
        MeaningListOrder objects in order of the
        MeaningList.meaning_id_list attribute.
        """
        for ml in orm.MeaningList.objects.all():
            meaning_id_list = [int(i) for i in ml.meaning_ids.split(",")]
            for i, meaning_id in enumerate(meaning_id_list):
                try:
                    m = orm.Meaning.objects.get(id=meaning_id)
                    orm.MeaningListOrder.objects.create(
                            meaning=m,
                            meaning_list=ml,
                            order=i+1)
                except orm.Meaning.DoesNotExist:
                    print "NOTE: MeaningList", ml.name, \
                            "--> Contained non-existent meaning id:", i
