...
def parse_source_csv(path):
  ...
  for row in csv_file:
    ...
    try:
      data_dict[int(entry[0])] = {'shorthand':entry[1], 'citation_text':entry[2]}
    except (IndexError, ValueError) as e:
      if e == IndexError:
        print 'Parsing error in line %s of %s: returns %s' %(i, path, entry)
      elif e == ValueError:
        print 'Parsing error in line %s of %s: %s is not a valid ID' %(i, path, entry[0])
    i+=1
  csv_file.close()
  return data_dict

def import_csv_citations(path):
  data_dict = parse_source_csv(path)
  for key in data_dict.keys():
    source_obj = Source.objects.get(pk=key)
