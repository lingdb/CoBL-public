...
def delete_sources(lst):
  for pk in lst:
    try:
      ...
      print('Deleting sources exception: source with id %s was already deleted' \
            %(pk))
...
def deprecate_sources(lst):
  for pk in lst:
    try:
      ...
      print('Deprecation sources handling exception: source with id %s is missing' \
            %(pk))
  ...
  for key in dic.keys():
    ...
    for pk in dic[key]:
        try:
          ...
          print('Duplicate sources handling exception: duplicate source with id %s was already deleted' \
                %(pk))
