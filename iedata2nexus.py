...
USE_DATA_BLOCK = True # set False to produce NeighborNets

def main():
    data = {} # key=CCN, value=[language list]
    languages, cogsets = [], []
    CCN = None
    for line in raw_data:
        if line.startswith("a"):
            ...
            CCN1, kind, CCN2 = [int(i) for i in line.split()[1:]]
            assert CCN1 > 99 and CCN2 > 99 # these should all be "doubtful"
            ...
            if CCN not in (0, 1) and CCN < 100: # i.e. informative, certain
                cogset = "%s-%s" % (meaning, CCN)
                if cogset not in cogsets:
                    cogsets.append(cogset)
                language = line[9:24].strip().replace(" ", "_")
                if language not in languages:
                    languages.append(language)
                data.setdefault(language, []).append(cogset)
        else:
            raise ValueError("Don't know what to do with line: "+line)
    maxlen = max(len(language) for language in languages)
    ...
    print(textwrap.dedent("""\
            ...
            [ vol. 82, part 5.                                                 ]
            [ Electronic format downloaded from:                               ]
            [   http://www.wordgumbo.com/ie/cmp/iedata.txt                     ]
            [ This nexus file excludes cognate sets coded as 'doubtful'        ]
            """))
    ...
    if USE_DATA_BLOCK:
        print(textwrap.dedent("""\
                begin data;
                  ...
                  matrix""" % (len(languages), len(cogsets))))
    else: # use taxa and characters blocks
        print(textwrap.dedent("""\
                begin taxa;
                  dimensions ntax=%s;
                end;
    ...
    for language in languages:
        ...
        for cogset in cogsets:
            if cogset in data[language]:
                row.append("1")
            else:
                row.append("0")
        print(language.ljust(maxlen), "".join(row))
    print(";\nend;")
    return
