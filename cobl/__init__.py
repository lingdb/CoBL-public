...
def configure(cfg_path):
    ...
    if cfg.has_option(section, 'clld.email_host'):
        settings.EMAIL_HOST = cfg.get(section, 'clld.email_host')

    if cfg.has_option(section, 'clld.app_email'):
        settings.SERVER_EMAIL = cfg.get(section, 'clld.app_email')

    if cfg.has_option(section, 'clld.admin_email'):
        settings.ADMINS = [(cfg.get(section, 'clld.admin_name'), cfg.get(section, 'clld.admin_email'))]
