...
from cobl.lexicon.models import CognateClass, \
                                 ...
                                 MeaningList, \
                                 Source
from cobl.lexicon.tests import make_basic_objects

# TODO make sure settings has DEBUG set to False

logger = logging.getLogger("unittest")
logger.setLevel(logging.INFO)
fh = logging.FileHandler("cobl/website/unittest.log", mode="w")
...
class MeaningListTests(TestCase):
    ...
    def test_reorder_view_down(self):
        from cobl.views import move_meaning
        meaning = self.meanings[0]
        orders = [(1, 0, 2, 3),
                  (1, 2, 0, 3),
                  (1, 2, 3, 0),
...
class LanguageListTests(TestCase):
    ...
    def test_reorder_view_down(self):
        from cobl.views import move_language
        language = self.languages[0]
        orders = [(1, 0, 2, 3),
                  (1, 2, 0, 3),
                  (1, 2, 3, 0),
...
class SearchTests(TestCase):
    ...
    def test_search_limit(self):
        ...
        from cobl.settings import LIMIT_TO
